...
interface AuthContextType {
  ...
  register: (name: string, email: string, password: string, emergencyContact: string) => Promise<User>;
  logout: () => void;
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Get current user info from protected route
        ...
        const user: User = {
          ...
          emergencyContact: res.data.emergencyContact ?? res.data.user?.emergencyContact ?? "",
        };
        setCurrentUser(user);
      } catch {
        // bad/expired token
  ...
  const register = async (name: string, email: string, password: string, emergencyContact: string): Promise<User> => {
    const res = await API.post("/auth/register", { name, email, password, emergencyContact }, {
      headers: { "Content-Type": "application/json" },
    });
    ...
    const normalized: User = {
      ...
      email: user.email,
      allergies: user.allergies ?? [],
      emergencyContact: user.emergencyContact ?? emergencyContact,
    };
