...
router.post("/register", async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      token,
      user: { id: newUser._id, name: newUser.name, email: newUser.email, allergies: newUser.allergies || [], emergencyContact: newUser.emergencyContact }
    });
  } catch (err) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    res.json({
      ...
      token,
      user: { id: user._id, name: user.name, email: user.email, allergies: user.allergies || [], emergencyContact: user.emergencyContact }
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
// UPDATE PROFILE (protected) - updates name, allergies, and emergencyContact
router.put("/profile", auth, async (req, res) => {
  try {
    const updates = {};
    if (typeof req.body.name === "string") updates.name = req.body.name;
    if (Array.isArray(req.body.allergies)) updates.allergies = req.body.allergies;
    if (typeof req.body.emergencyContact === "string") updates.emergencyContact = req.body.emergencyContact;
